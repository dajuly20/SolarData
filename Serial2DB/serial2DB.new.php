...
// Request Data from shunt module on my own?
// If the Display-module is online as well, set to false.
$requestData = true; 

// Define string for data request
$sendRequestStr = chr(hexdec("AA")).chr(hexdec("04")).chr(hexdec("01")) .chr(hexdec("01")). chr(hexdec("B0"))."";

/*
  Warning -- ab hier in DOKU
*/

//Throws error and shuts down if not found.
require 'PHP-Serial/src/PhpSerial.php';

// Initialise serial class
$serial = new PhpSerial;

// First a device has to be specified. This works on both linux and windows 
//(if the linux serial device is /dev/ttyS0 for COM1, etc)
$serialPortUri = "/dev/ttyUSB0";
$serial->deviceSet($serialPortUri);

// Baud rate, parity, length, stop bits, flow control are changable.
...
while($continueLoop){
	...
	{  
		...
		if($charsRead >= 28){
			$readSwitch = false;
			$charsRead  = 0;
			evaluateData($data, $lastDataSet, $myslqi);
...
function evaluateData($data, &$lastDataSet, &$mysqli){
	
	// Convert from Hex!
	$currentDataSet = convertFromHex($data);
...
function sanityCheck($c,$o,&$isFirst){
	...
	$ePow  = !($c["pow"] >=   -20*1000	&&	$c["pow"] <= 2000*1000) ;  		// Min is 140W Panwl / 12V = 11 +x = 20A . Max: 1500W Inverter + x = 2000   
	...
	$eChr = !percentCompare($c,$o,"chg",$driftRange = 5, $zeroRange = 30);
...
function percentCompare($currentArray,$oldArray,$index,$driftRange = 30, $zeroRange = 50){
	...
	if($oVal == $cVal){
	//trigger_error (date("m.d.y H:i:s")." value <$index> '$cVal' is identical to '$oVal' [LastVal: $oVal; drift range: $driftRange; zero range: $zeroRange]" , E_USER_NOTICE );
	...
	if($crossZero){ $eAdd .="crossZero";}
		...
		$isValid =  ($valSmall + abs($zeroRange)) >= $valBig;
	}
	
	
	if(!$isValid){
		if($GLOBALS["verbosity"] >= 1) trigger_error (date("m.d.y H:i:s")." value <$index> '$cVal' discarded for variation on $eAdd range. [LastVal: $oVal; TimeOffset: $timeOffset; drift range: $driftRange; zero range: $zeroRange]" , E_USER_WARNING );
		return false;
	}else{
		return true;
	}
