...
function sig_handler($signo) 
{
     ...
     switch ($signo) {
         ...
         case SIGHUP:
			...
			break;
         case SIGUSR1:
             echo "SIGUSR1 erkannt...\n";
			 $GLOBALS["continueLoop"] = false;
             break;
		case SIGINT:
			 echo "\nSIGINT erkannt... exit nicely! :-)\n";
			 $GLOBALS["continueLoop"] = false;
             break;
         default:
...
trigger_error (date("m.d.y H:i:s"). " Started Script. Verbosity: $verbosityStr Sending AA040101B0 to shunt module.  " , E_USER_NOTICE );
echo "\n\nStarted & Running!\nCTRL + C to quit.\n";
...
function sanityCheck($c,$o,&$isFirst){
	...
	$ePow  = !($c["pow"] >=    0*1000	&&	$c["pow"] <= 2000*1000) ;  		// Power is always positive. Max: 1500W Inverter + x = 2000   
	$eChg  = !($c["chg"] >= -500*1000	&&	$c["chg"] <=  500*1000) ;  		// Capacity of Battery is 250 Ah. this value doubled should be sufficient.     
	$eMul  = !(($c["vlt"] * abs($c["cur"])) == $c["pow"] )          ;  		// Voltage times *absolute* current must equal the power.
	...
	if($eVlt || $eCur || $ePow || $eChg ||$eMul){
		...
		if($verbosity <= 1) trigger_error (date("m.d.y H:i:s")." dataset discarded. Did not pass sanity check on: $eAdd " , E_USER_WARNING );
		return false;
	}
...
function percentCompare($currentArray,$oldArray,$index,$driftRange = 30, $zeroRange = 50){
	...
	if(!$isValid){
		if($verbosity >= 1) trigger_error (date("m.d.y H:i:s")." value <$index> '$cVal' discarded for variation on $eAdd range. [LastVal: $oVal; TimeOffset: $timeOffset; drift range: $driftRange; zero range: $zeroRange]" , E_USER_WARNING );
		return false;
	}else{
		return true;
	}
